...
def getPendingByStage():
    ...
    isofficiating = handlerInfo.isOfficiating
    print(isofficiating)
    if stageCurrent==14:
        print(stageCurrent)
        ltcInfos = LTCInfo.query.filter(LTCInfo.stageCurrent == 1).all()
    else:
    ...
    if(stageCurrent == 14 and isofficiating):
        print("komal")
        return [ltc.json() for ltc in ltcInfos if ltc.user.department == handlerInfo.department]
    if(stageCurrent == 14 and isofficiating==False ):
        return []
...
def schedule_hod_dates(email, start_date, end_date):
        ...
        scheduler.add_job(
            ...
            trigger=DateTrigger(run_date=run_time1+ timedelta(minutes=2) ),
...
def submitHodData():
    ...
    if (status == 'ACCEPT'):
        ...
        ltcInfo.rejectedBy = handlerInfo.roleId
    ...
    if(handlerInfo.roleId!=1):
        for roleno in range(2,handlerInfo.roleId):
            if(roleno==handlerInfo.roleId):
                continue
            user = User.query.filter(User.roleId == roleno).first()
            appinfo = user.notifications.append(Notification(message))
        
        user1 = User.query.filter(User.department == ltcInfo.user.department , User.roleId == 1 ).first()
        print(ltcInfo.user.department)
        appinfo1 = user1.notifications.append(Notification(message))
...
def submitTAHodData():
    ...
    if (status == 'ACCEPT'):
        ...
        taInfo.rejectedBy = handlerInfo.roleId
    ...
    if(handlerInfo.roleId!=1):
        ...
        user1 = User.query.filter(User.department == taInfo.user.department , User.roleId == 1 ).first()
        print(taInfo.user.department)
        appinfo1 = user1.notifications.append(Notification(message))
    print([comment.json() for comment in taInfo.comments])
    # # print(ltcInfo.json())
    db.session.commit()
