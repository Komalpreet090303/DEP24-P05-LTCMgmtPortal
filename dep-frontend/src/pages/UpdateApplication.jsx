...
export default function UpdateApplication() {
  ...
  const [from, setFrom] = useState(ltcInfo[0].fromDate);
  const [to, setTo] = useState(ltcInfo[0].toDate);
  const [suffixFrom, setSuffixFrom] = useState(ltcInfo[0].suffixFrom);
  const [suffixTo, setSuffixTo] = useState(ltcInfo[0].suffixTo);
  const [prefixFrom, setPrefixFrom] = useState(ltcInfo[0].prefixFrom);
  const [prefixTo, setPrefixTo] = useState(ltcInfo[0].prefixTo);
  const [encash, setEncash] = useState(ltcInfo[0].encashmentAvailed);
  const [ud, setud] = useState(ltcInfo[0].fromDate);
  const {id} = useParams();
  const receiptRef = useRef(null);
  const [earnedLeave, setEarnedLeaveValue] = useState(ltcInfo[0].earnedLeaveAvailed)
  const [encashmentDays,setEncashmentDays] = useState(ltcInfo[0].encashmentNoOfDays);
  const [natureVisitingPlace, setnatureVisitinglace] = useState(ltcInfo[0].natureOfTravel);
  const [visitPlace, setvisitingPlace] = useState(ltcInfo[0].placeToVisit);
  const [fare,setfare] = useState(ltcInfo[0].totalEstimatedFare);
  ...
  const handleInfo = (json) => {
    setLtcData(json);
    console.log(ltcData.earnedLeaveAvailed)
    setEarnedLeaveValue(json.earnedLeaveAvailed)
    console.log(earnedLeave)
    console.log(ltcData.fromDate);
    setEncash(json.encashmentAvailed)
    setEncashmentDays(json.encashmentNoOfDays)
    setFrom(json.fromDate)
    setPrefixFrom(json.prefixFrom)
    setPrefixTo(json.prefixTo)
    setTo(json.toDate)
    setSuffixFrom(json.suffixFrom)
    setSuffixTo(json.suffixTo)
    setnatureVisitinglace(json.natureOfTravel)
    setvisitingPlace(json.placeToVisit)
    setfare(json.totalEstimatedFare)
  ...
  const ltcSubmitHandler = (e) => {
    ...
    formData["ltcId"] = id; // Add LTC ID obtained from params
    ...
    fetch("/api/updateNewLTCApplications", {
    ...
    <div className="bg-yellow-50 h-screen overflow-auto">
      <div className="max-w-screen-xl mx-auto">
        <h3 className="font-semibold text-xl text-gray-900 p-4 flex mx-auto">
          Update LTC Application
        </h3>
        {/* // <>
        // <Input  name="earnedLeave"
        // type="number"
        // value={earnedLeave}
        //  />
        // </> */}
        ...
        > 
          <InputGroup>
            ...
            >
              ...
              {"Earned Leave Availed"}</label>
        {true ? <span className="text-red-500 font-extrabold"> * </span> : null} 
        <input  name="earnedLeave"
                type="number"
                value="3" /> */}
              ...
              <Input
                ...
                iscompulsory={true}
                required
                onChange={(e) => setEarnedLeaveValue(e.target.value)}
              />
              <Input
                label={"From"}
                name="leaveFrom"
            ...
            <div className="m-4 grid gap-6 mb-1 md:grid-cols-2 xl:grid-cols-4">
              <Input
                ...
                name="prefixFrom"
                type="date"
                value={prefixFrom ? new Date((prefixFrom))
                  .toISOString()
                  .substring(0, 10) : ""}
              ...
              <Input
                ...
                name="prefixTo"
                type="date"
                value={prefixTo ? new Date((prefixTo))
                  .toISOString()
                  .substring(0, 10) : ""}
            ...
            >
              <Input
                ...
                name="suffixFrom"
                type="date"
                value={suffixFrom ? new Date((suffixFrom))
                  .toISOString()
                  .substring(0, 10) : ""}
              ...
              <Input
                ...
                name="suffixTo"
                type="date"
                value={suffixTo ? new Date((suffixTo))
                  .toISOString()
                  .substring(0, 10) : ""}
...
xl:grid-cols-4"
            >
              ...
              <Input
                ...
                name="visitNature"
                type="text"
                value={natureVisitingPlace}
                onChange={(e) => {setnatureVisitinglace(e.target.value)}}
                iscompulsory={true}
                required
              />
              <Input
