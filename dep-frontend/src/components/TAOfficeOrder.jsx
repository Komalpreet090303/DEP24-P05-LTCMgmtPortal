...
export default function TAOfficeOrder({ taInfo }) {
  ...
  const [ltcData, setLtcData] = useState((JSON.parse(window.localStorage.getItem("TA")) || {}).ltcInfo);
  useEffect(() => {
    const handleStorageChange = () => {
      const ltc = JSON.parse(window.localStorage.getItem("TA")) || {};
      setLtcData(ltc.ltcInfo);
    };
