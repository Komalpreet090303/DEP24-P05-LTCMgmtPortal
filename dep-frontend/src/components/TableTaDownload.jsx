...
export default function TableTaDownload({ fields, data, setData, readOnly = false }) {
  ...
  const addNewRow = () => {
    var addNewRow = true;//changed const to var
  ...
  return (
    <>
      <div className="relative  shadow-md sm:rounded-lg pb-2 pl-2 mb-2 bg-white" >
        <table className="w-full text-base text-left text-gray-900">
          <thead className="text-base border-b border-gray-200 text-gray-700 whitespace-nowrap">
            <tr>
              {fields.map((field) => (
                <th scope="col" className="py-3 font-semibold text-sm">
                  {field.heading}
                </th>
              ))}
            </tr>
